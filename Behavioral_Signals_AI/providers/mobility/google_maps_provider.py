"""Future Google Maps Trends aggregate signal provider."""
...
class GoogleMapsAggregateProvider:
    provider_name = "google_maps"
    provider_type = "mobility"
    source_label = "Google Maps Trends"
    ...
    def fetch_signals(self, location: str = "Kenya", limit: int = 10) -> ProviderResult:
        status = ProviderStatus(self.provider_name, self.provider_type, False, self.is_available(), self.provider_name, "Google Maps Trends requires a formal aggregate data integration and is not active in Phase 1.")
